            ...
            if (act.isMacro()) {
                ...
                if(dataCount == 3 || dataCount == 4){
                    ...
                    for(int i = 0; i < macroLines.length(); i++){
                        ...
                        insertIntoMacroPreview(ml.keyDown, ml.printable, ml.key);
                    }
...
void RebindWidget::on_btnStartMacro_clicked() {
    ...
    if (macroReader) {
        ...
        ui->lbl_macro->setText(tr("Click Apply or manually edit Macro Key Actions by double clicking."));
...
{
    macroLines.setDefaultDelay(false);
}

void RebindWidget::on_rb_delay_default_toggled(bool checked)
{
    macroLines.setDefaultDelay(true);
}
...
void RebindWidget::macroLineRead(QString line, qint64 ustime, bool keydown){
    ...
    macroLines.append(macroLine(line, ustime, MACRO_DELAY_DEFAULT, keydown, printable));
...
void RebindWidget::on_captureTypeBox_currentIndexChanged(int index){
    ...
    msg.setText(tr("The \"Record from all keyboards\" recording method is only recommended if you do not have a keyboard managed by ckb-next.\n"
                   "It currently only functions correctly with an English - United States keyboard layout.\n"
                   "Make sure your keyboard is switched to it before starting a recording."));
